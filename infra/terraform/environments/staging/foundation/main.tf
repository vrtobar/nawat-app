...
resource "aws_s3_bucket_cors_configuration" "assets" {
  ...
  cors_rule {
    allowed_methods = ["PUT"]
    allowed_origins = [
      "https://staging.nahuat.com",
      # Local development points at staging's API, so a presign issued there
      # names staging's bucket and the browser's origin is localhost. Listed
      # here and deliberately not in production, which no development server
      # has any business uploading to.
      "http://localhost:3000",
    ]
